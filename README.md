...
- **♻️ Reuse an existing world** — already generated a world with your key? In the same dialog, paste its **request ID** (`req_…`) or **world link / ID** (`app.spaitial.ai/worlds/<id>`) and Import. It pulls that world's splat **and mesh** straight from the API — no regeneration. Request IDs are instant; a world ID is resolved by looking it up among your recent requests.
