...
- **♻️ Reuse an existing world** — already generated a world with your key? In the same dialog, paste its **request ID** (`req_…`, returned when it was created) and Import. It pulls that world's splat **and mesh** straight from the API — no regeneration.
